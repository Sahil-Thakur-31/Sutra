import { NextResponse, type NextRequest } from "next/server";
import { ObjectId } from "mongodb";
import { getDb } from "@/lib/mongo/client";
import { getUidFromRequest } from "@/lib/auth/session";
import { requireMembership } from "@/lib/household/membership";
import type { FundDoc, ContributionDoc } from "@/lib/household/fund";

export async function DELETE(
  request: NextRequest,
  { params }: { params: Promise<{ id: string; fundId: string }> }
) {
  const uid = getUidFromRequest(request);
  if (!uid) return NextResponse.json({ error: "Not signed in." }, { status: 401 });

  const { id: householdId, fundId } = await params;
  if (!ObjectId.isValid(fundId)) return NextResponse.json({ error: "Invalid fund id." }, { status: 400 });

  const db = await getDb();
  const household = await requireMembership(db, householdId, uid);
  if (!household) return NextResponse.json({ error: "Household not found." }, { status: 404 });

  const result = await db.collection<FundDoc>("funds").deleteOne({ _id: new ObjectId(fundId), householdId });
  if (result.deletedCount === 0) return NextResponse.json({ error: "Fund not found." }, { status: 404 });

  await db.collection<ContributionDoc>("fundContributions").deleteMany({ householdId, fundId });

  return NextResponse.json({ ok: true });
}
