...
import { toReminderLogDTO, type ReminderLogDoc } from "@/lib/household/reminder";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const docs = await db
    .collection<ReminderLogDoc>("reminderLogs")
    .find({ householdId, dismissedAt: { $gte: range.start, $lt: range.end } })
    .sort({ dismissedAt: -1 })
    .toArray();

  return NextResponse.json({ logs: docs.map(toReminderLogDTO) });
}
