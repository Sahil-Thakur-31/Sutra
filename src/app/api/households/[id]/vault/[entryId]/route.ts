...
import type { VaultEntryDoc } from "@/lib/household/vaultEntry";
...
export async function DELETE(
  ...
  const { id: householdId, entryId } = await params;
  if (!ObjectId.isValid(entryId)) return NextResponse.json({ error: "Invalid entry id." }, { status: 400 });
  ...
  const result = await db
    ...
    .deleteOne({ _id: new ObjectId(entryId), householdId });
  if (result.deletedCount === 0) return NextResponse.json({ error: "Entry not found." }, { status: 404 });

  publish(householdId, { type: "vault-removed", entryId });
